...
var arrayProto = Array.prototype;//Array构造函数的原型对象{map,push,unshift}
var arrayMethods = Object.create(arrayProto);//新对象继承了数组构造方法的原型对象
...
methodsToPatch.forEach(function (method) {
  ...
  // 'push'
  var original = arrayProto[method];//获取原来最初始的方法 'sort'
  ...
  def(arrayMethods, method, function mutator() {
    ...
    //push(1213)
    //[3434343,]
    var result = original.apply(this, args);//拿到结果  1213
    //[3434343,1213]


    var ob = this.__ob__;//当前observer
...
Observer.prototype.walk = function walk (obj) {
   ...
   */
  var keys = Object.keys(obj);//[time,yidneg]
...
function createASTElement(
  ...
  return {
    type: 1, //节点类型
    tag: tag, //节点名称
    attrsList: attrs, //节点属性数组
    attrsMap: makeAttrsMap(attrs), //节点属性映射
    rawAttrsMap: {}, //原始属性mas
    parent: parent, //父节点
    children: [] //子节点
  }
...
function parse(
  ...
  parseHTML(template, {
    ...
    start: function start(tag, attrs, unary, start$1, end) {
      ...
      //每次一个开始标签的时候，都调用一个对应的AST构建
      //构建一个基本的AST的基础模板，后面通过它来不断的搭建
    ...
    //标签结束时的回调
      ...
      // pop stack
      stack.length -= 1;//出栈
      currentParent = stack[stack.length - 1]; //获取出栈之后的栈顶元素，就是当前元素的父节点
      if ( options.outputSourceRange) {
        element.end = end$1;
      }
      closeElement(element);
    ...
    chars: function chars(text, start, end) {
      ...
      if (text) {
        ...
        if (!inVPre && text !== ' ' && (res = parseText(text, delimiters))) {
          ...
          child = {
            type: 2,
            expression: res.expression, //_s(name)
            tokens: res.tokens,
            text: text
          };
...
function createCompileToFunctionFn(compile) {
  ...
  return function compileToFunctions(
    ...
    var res = {};
    var fnGenErrors = [];
    //编译拼接出来的字符串，new Function为函数
    /**
     * compiled
     * {
     *  render:string==>函数字符串
     *  staticRenderFns：优化之后静态节点编译的函数字符串
     * }
     */
...
function createCompilerCreator(baseCompile) {
  ...
  return function createCompiler(baseOptions) {
    ...
    return {
      compile: compile,
      compileToFunctions: createCompileToFunctionFn(compile)//编译方法
    }
  }
}
...
function createRenderer(ref) {
  ...
  return {
    renderToString: function renderToString(
      ...
      if (context) {
        templateRenderer.bindRenderFns(context);//注入静态资源
      }
      ...
      try {
        render(component, write, context, function (err) {
          ...
          if (template) {
            try {
              var res = templateRenderer.render(result, context);//写用户其他的参数
...
function compileModule(files, basedir, runInNewContext) {
  ...
  function getCompiledScript(filename) {
    if (compiledScripts[filename]) {
      return compiledScripts[filename]//已经执行过的js
    }
  ...
  function evaluateModule(filename, sandbox, evaluatedFiles) {
    ...
    if (evaluatedFiles[filename]) {
      return evaluatedFiles[filename]  //module ==>结果
    }
  ...
  return function createBundleRenderer(
    ...
    */
    ...
    if (typeof bundle === 'object') {
      entry = bundle.entry;//"main.js"
      files = bundle.files;//{ "main.js":'代码'}
    ...
    var renderer = createRenderer(rendererOptions);//创建内部渲染器
